 ...
 ********)
 ...
 | AST_binop of (string * ast_e * ast_e * row_col);;
  ...
  match p with
  | PT_nt ("P",_, [sl; PT_term("$$", _)])
      -> ast_ize_stmt_list sl
...
and ast_ize_stmt_list (sl:parse_tree) : ast_sl =
  ...
  (*
    NOTICE: your code here
  *)
  | PT_nt ("SL", _, [s; PT_term(";",_) ; sl])
      -> (ast_ize_stmt s) @ (ast_ize_stmt_list sl)
...
and ast_ize_stmt (s:parse_tree) : ast_sl =
  ...
  |PT_nt("S",_,[PT_term("if",_); expr; PT_term("then", _); sl; PT_term("end",_)])
    -> [AST_if(ast_ize_cond expr, ast_ize_stmt_list sl)]
  |PT_nt("S",_,[PT_term("while",_); expr; PT_term("do", _); sl; PT_term("end",_)])
    -> [AST_while(ast_ize_cond expr, ast_ize_stmt_list sl)]
  | _ -> raise (Failure "malformed parse tree in ast_ize_stmt")


and ast_ize_expr (e:parse_tree) : ast_e =   (*E, T, or F *)
  match e with
  |PT_nt("F",_,[PT_int(lhs, vloc)])
    -> AST_int(lhs,vloc)
  |PT_nt("F",_,[PT_real(lhs, vloc)])
    -> AST_real(lhs,vloc)
  |PT_nt("F",_,[PT_id(lhs, vloc)])
    -> AST_id(lhs,vloc)
  |PT_nt("F",_,[PT_term("(", vloc); expr; PT_term(")",_)])
    -> ast_ize_expr expr
  |PT_nt("F",_,[PT_term("trunc", _); PT_term("(", vloc); expr; PT_term(")",_)])
    -> AST_trunc(ast_ize_expr expr, vloc)
  |PT_nt("F",_,[PT_term("float", _); PT_term("(", vloc); expr; PT_term(")",_)])
...
and ast_ize_expr_tail (lhs:ast_e) (tail:parse_tree) : ast_e =   (* TT, or FT *)
  ...
  (*
    NOTICE: your code here
  *)
  | PT_nt ("TT", _, [PT_nt("AO", _ ,[PT_term (ao, lhc)]); t; tt])
        -> ast_ize_expr_tail (AST_binop (ao, lhs, ast_ize_expr t, lhc)) tt
  | PT_nt ("FT", _, [PT_nt("MO", _ ,[PT_term (mo, lhc)]); f; ft])
        ->  ast_ize_expr_tail (AST_binop (mo, lhs, ast_ize_expr f, lhc)) ft
  | PT_nt ("TT", _, [])
        -> lhs
  | PT_nt ("FT", _, [])
        -> lhs
...
and ast_ize_cond (c:parse_tree) : ast_e = (*RO*)
  match c with
  (*
    NOTICE: your code here
  *)
  |PT_nt("C",lhc, [ expr1; PT_nt("RO", _, [PT_term("<>",_)]); expr2])
      ->  AST_binop ("<>" , ast_ize_expr expr1, ast_ize_expr expr2, lhc)
  |PT_nt("C",lhc, [ expr1; PT_nt("RO", _, [PT_term("==",_)]); expr2])
      ->  AST_binop ("==" , ast_ize_expr expr1, ast_ize_expr expr2, lhc)
  |PT_nt("C",lhc, [ expr1; PT_nt("RO", _, [PT_term("<",_)]); expr2])
      ->  AST_binop ("<" , ast_ize_expr expr1, ast_ize_expr expr2, lhc)
 ...
 ********)
 ...
 type scope_info =
   { (*
         ...
         bool = true then is variabels
         bool = false then is temp_variabels
     *)
     variables : (string * tp * int * bool) list
   };;
 type symtab =
   {
    (*Scopes consist of scope_info and the scope number*)
     max_temp : int;
     max_mem : int;
     marker_num : int; (*Keep track of how many scope existed in this symtable*)
     scopes     : scope_info list
   };;
 let empty_symtab = {max_temp = 0; max_mem = 0; marker_num = 0; scopes = []};;

 (* Open a new scope, in which variable names can be reused. *)
 let new_scope (st:symtab) : symtab =
   { max_temp = st.max_temp; max_mem = st.max_mem; marker_num = st.marker_num; scopes     = { variables = []} :: st.scopes };;
 ...
 let end_scope (st:symtab) : symtab =
   ...
   (*Ending the current scope updates the symtab memory*)
   { max_temp = st.max_temp; max_mem = st.max_mem; marker_num = st.marker_num; scopes     = ss }
 ...
 *)
 ...
 let name_match_st id = fun (sym, _, _, _) -> id = sym;;

 (*
  create memory for current scope by scaning all scope
 *)
 let new_mem_addr (id:string) (st:symtab) (typ:bool): int =
  let rec all_scopes (id1:string) (initial:int) scope_stack (typ_1:bool): int =
    match scope_stack with
    | [] -> initial
    | scope :: surround ->
      let rec helper (id2: string) (initial2:int) (typ_n:bool) (sp) : int =
        match sp with
        | [] -> initial2
        | h :: tail ->
          let (id_temp, _, _, ty) = h in
          if (id_temp = id2 && ty = typ_n) then initial2 else if ty = typ_n then helper id2 (initial2+1) typ_n tail else helper id2 initial2 typ_n tail in
    all_scopes id1 (initial+(helper id1 0 typ_1 scope.variables)) surround typ_1 in
  all_scopes id 0 st.scopes typ;;
 let max_mem_addr (st:symtab) = st.max_mem;;
 ...
 let insert_st (id:string) (t:tp) (st:symtab) (temp:bool): symtab * bool =
   ...
   | scope :: surround ->
       ...
       | Some _ -> (st, false)
       | None   ->
           ({max_temp = if temp = false then (st.max_temp+1) else st.max_temp; max_mem = if temp then (st.max_mem+1) else st.max_mem; marker_num = st.marker_num; scopes     =
                { variables = (id, t, new_mem_addr id st temp, temp) :: scope.variables }
                :: surround },
            true);;

 (* Look id up in symbol table.  If not found, insert to limit redundant
    subsequent errors. *)
 let lookup_st (id:string) (st:symtab) (loc:row_col) (temp: bool)
     : tp * string * string * symtab =
     (* type, target-code name, error msg, new symtab *)
   let rec helper scope_stack =
     match scope_stack with
     | [] -> let (new_st, stat) = (insert_st id Unknown st temp) in
     ...
     | scope :: surround ->
         ...
         | Some (_, t, a, temp) ->
            (match t with
             | Int     -> if temp then (t, ("i[" ^ Int.to_string a ^ "]"), "", st) else (t, ("ti[" ^ Int.to_string a ^ "]"), "", st)
             | Real    -> if temp then (t, ("r[" ^ Int.to_string a ^ "]"), "", st) else (t, ("tr[" ^ Int.to_string a ^ "]"), "", st)
             | Unknown -> (t, "", "Something Wrong", st))   (* already complained *)
         | None -> helper surround in
   helper st.scopes;;

   let increment (st:symtab) (increment_num:int) : symtab = ({max_temp = st.max_temp; max_mem = st.max_mem; marker_num = (st.marker_num+increment_num); scopes = st.scopes})
 (********
 ...
 ********)
 ...
 and translate_s (s:ast_s) (st:symtab)
   ...
   (*
     NOTICE: your code here
   *)
   | AST_i_dec (sl, (a,b)) -> let (st1, tf) = insert_st sl Int st true in
                              (*Memeory Increment*)
                              if tf then (st1, [], []) else (st1, [], ["Redeclaration is prohibited at location"^Int.to_string a^Int.to_string b])(*declare a*)
   | AST_r_dec (sl, (a,b)) -> let (st1, tf) = insert_st sl Real st true in
                              if tf then (st1, [], []) else (st1, [], ["Redeclaration is prohibited at location"^Int.to_string a^Int.to_string b])
   | AST_read(sl, loc) -> translate_read sl loc st
   | AST_write(e) -> translate_write e st
   | AST_assign(sl, e, loc1, loc2) -> translate_assign sl e loc1 loc2 st
   | AST_if (e, s2) -> translate_if e s2 st
   | AST_while(e, s2) -> translate_while e s2 st
   | _ -> st, [], []

 and translate_read (id:string) (loc:row_col) (* of variable *) (st:symtab)
     : symtab * string list * string list =
     (* new symtab, code, error messages *)
    let (tp, tcn, emsg, st2) = lookup_st id st loc true in
    let emsg = if emsg = "" then [] else [emsg] in
   (*
     NOTICE: your code here
   *)
   match tp with
   | Int -> (st2, [tcn^" = getint();"], [])
   | Real -> (st2, [tcn^" = getreal();"], [])
   | Unknown -> (st, [], emsg)
 ...
 and translate_write (expr:ast_e) (st:symtab)
   ...
   let (st_n, tp, code, self, emsg) = translate_expr expr st in
   (*Join the prev code and message*)
   let message =
    match self.kind with
    | Atom -> self.text
    | Temp(number) ->
      if tp = Int then "ti["^Int.to_string number^"]"
      else if tp = Real then "tr["^Int.to_string number^"]"
      else "" in
    let (st_temp,_) = insert_st message tp st_n false in
    let st1 = if self.kind <> Atom then st_temp else st_n in
    if tp = Int then (st1, (if self.kind = Atom then code@["putint("^message^");"] else code@[message^" = "^self.text]@["putint("^message^");"]), emsg)
    else if tp = Real then (st1, (if self.kind = Atom then code@["putreal("^message^");"] else code@[message^" = "^self.text]@["putreal("^message^");"]), emsg)
    else (st1, [], emsg)
   (*
     NOTICE: your code here
   *)
 ...
 and translate_assign (id:string) (rhs:ast_e) (vloc:row_col) (aloc:row_col) (st:symtab)
     : symtab * string list * string list =
     (* new symtab, code, error messages *)
  let (tp1, tcn, emsg, st1) = lookup_st id st vloc true in
  let (st2, tp, code, self, emsg1) = translate_expr rhs st1 in
  let emsg = if emsg = "" then [] else [emsg] in
  let (a,b) = aloc in
   (*
     NOTICE: your code here
   *)
   if tp = Unknown then (st2, [], emsg@emsg1)
   else if tp1 <> tp then (st, [], emsg@emsg1@["type clash error at line "^Int.to_string a^" col "^Int.to_string b])
   else (st2, code@[tcn^" = "^self.text^";"], emsg@emsg1)
 ...
 and translate_if (c:ast_e) (sl:ast_sl) (st:symtab)
   ...
   | AST_binop(_, _, _, _) ->
      let (n_st, tp, code, self, emsg) = translate_expr c st in (*Handle nested expression*)
      let message =
        match self.kind with
        | Atom -> self.text
        | Temp(number) ->
      ...
      let (st_temp,_) = insert_st message tp n_st false in
      let st0 = if self.kind <> Atom then st_temp else n_st in
      (*new scopes are added infront of list*)
      let st1 = increment(new_scope st0) 1 in
      let (st2, sl_code, emsg2) = translate_sl sl st1 in
      (*Marker of the previous scope*)
      (*code@[if !essmage goto L(Int)]@[sl]@[L(Int)]*)
      let m_num = "L"^Int.to_string (st1.marker_num-1) in
      (end_scope st2, [message^" = "^self.text^";"]@code@["if (!"^message^") goto "^m_num^";"]@sl_code@[m_num^":"], emsg@emsg2)
 ...
 and translate_while (c:ast_e) (sl:ast_sl) (st:symtab)
    ...
    | AST_binop(_, _, _, _) ->
      ...
      let st0 = if self.kind <> Atom then st_temp else n_st in
      (*new scopes are added infront of list*)
      let st1 = increment (new_scope st0) 2 in
      let (st2, sl_code, emsg2) = translate_sl sl st1 in
      (*this scope, next scope+1*)
      (end_scope st2, ["L"^Int.to_string (st1.marker_num-1)^":"]@code@[message^" = "^self.text^";"]@["if (!"^message^") goto L"^Int.to_string (st1.marker_num-2)^";"]@sl_code@["goto L"^Int.to_string (st1.marker_num-1)^";"]@["L"^Int.to_string (st1.marker_num-2)^":;"], emsg@emsg2)
    | _ -> raise (Failure "unexpected expression type as condition")
 ...
 and translate_expr (expr:ast_e) (st:symtab)
   ...
   | AST_id(i, loc)             -> let (tp, tcn, emsg, st1) = lookup_st i st loc true in
                                    let new_emsg = if emsg = "" then [] else [emsg] in
                                    (st1, tp, [], {text = tcn; kind = Atom}, new_emsg)
   | AST_float(e, (a,b))            -> let (nst, tp, s_code, self, emsg) = translate_expr e st in
                                    if tp = Real then (nst, tp, s_code, self, ["argument of float must be an int at line"^Int.to_string a^" col "^Int.to_string b]) else
                                    (match self.kind with
                                      | Atom -> (nst, Real, [], {text = "to_real("^self.text^");"; kind = Temp(0)}, [])
                                      | Temp(number) -> let (st_new, _) = insert_st ("tr["^Int.to_string (number+1)^"]") Real nst false in
                                                let prev =
                                                    if tp = Real then "tr["^Int.to_string number^"]"
                                                    else "ti["^Int.to_string number^"]" in
                                                (st_new, Real, s_code@[prev^" = "^self.text^";"], {text = "to_real("^prev^")"; kind = Temp(number+1)}, []))
                                                      (*Find register using temp register number and assign this number to this expression*)
   | AST_trunc(e, (a,b))            -> let (nst, tp, s_code, self, emsg) = translate_expr e st in
                                    if tp = Int then (nst, tp, s_code, self, ["argument of trunc must be an float at line"^Int.to_string a^" col "^Int.to_string b]) else
                                    (match self.kind with
                                      | Atom -> (nst, Int, [], {text = "to_int("^self.text^");"; kind = Temp(0)}, [])
                                      | Temp(number) -> let (st_new, _) = insert_st ("ti["^Int.to_string (number+1)^"]") Real nst false in
                                                let prev =
                                                  if tp = Real then "ti["^Int.to_string number^"]"
                                                  else "ti["^Int.to_string number^"]" in
                                                (st_new, Int, s_code@[prev^" = "^self.text^";"], {text = "to_int("^prev^")"; kind = Temp(number+1)}, []))
                                  (*Match first expression and check if its a temp, do the same for second expression*)
   | AST_binop(i, e1, e2, (a,b))  -> let (st1, tp, code, self, emsg) = translate_expr e1 st in
                                 let (st2, tp2, code2, self2, emsg2) = translate_expr e2 st1 in
                                 (*self.text should looks like 1 + 2, self.temp is going to contain the temporary register number*)
                                 (*Join previous statment*)
                                 let typ1 =
                                  match self.kind with
                                  | Atom -> -1
                                  | Temp(num) -> num+1 in
                                 let typ2 =
                                  match self2.kind with
                                  | Atom -> -1
                                  | Temp(num) -> num+1 in

                                let regist = fun x num -> if x = Real then "tr["^Int.to_string num^"]" else "ti["^Int.to_string num^"]" in
                                let new_code = if self.kind <> Atom then code@[regist tp (typ1-1)^" = "^self.text^";"] else code in
                                let new_code2 = if self2.kind <> Atom then code2@[regist tp2 (typ2-1)^" = "^self2.text^";"] else code2 in
                                 (*case in *)
                                 match i with
                                 | "/" -> (*code: Just previous code, text : binop text, kind = tempRegist*)
                                          let context = if tp = Int then ("divide_int") else ("divide_real") in
                                          let context2 =
                                            (if typ1 = -1 && typ2 = -1 then 0 (*Atomic Atomic*)
                                            else
                                              (if typ1 > typ2 then typ1
                                              else if typ2 > typ1 then typ2
                                              else typ1)) in
                                          let one = if self.kind <> Atom then regist tp (typ1-1) else self.text in
                                          let two = if self2.kind <> Atom then regist tp (typ2-1) else self2.text in

                                          if tp = tp2 then (st2, tp, new_code@new_code2,
                                            {text = (context^"("^one^", "^two^");"); kind = Temp(context2)}, emsg@emsg2)(*operation*) (*lambda and lambda*)
                                          else (st, Unknown, new_code@new_code2, {text = i; kind = Atom}, ["Type Mismatch error at line "^Int.to_string a^" col "^Int.to_string b])(*throw an error*)
                                 | _ ->
                                      let context = if i = "<>" then "!=" else i in
                                      let context2 =
                                        (if typ1 = -1 && typ2 = -1 then 0 (*Atomic Atomic*)
                                        else
                                          (if typ1 > typ2 then typ1
                                      ...
                                      let condition = if (i = "<>" || i = "<" || i = ">" || i = "==" || i = "<=" || i = ">=") then Int else tp in
                                        let one = if self.kind <> Atom then regist tp (typ1-1) else self.text in
                                        let two = if self2.kind <> Atom then regist tp (typ2-1) else self2.text in
                                        if tp = tp2 then (st2, condition, new_code@new_code2,
                                          {text = (one^" "^context^" "^two); kind = Temp(context2)}, emsg@emsg2)
                                      else (st, Unknown, new_code@new_code2, {text = i; kind = Atom}, ["Type Mismatch error at line "^Int.to_string a^" col "^Int.to_string b])
